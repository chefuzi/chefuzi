...
namespace CheFuZi.Controllers
{
    ...
    public class ManageBookClickController : Controller
    {
        #region 内容列表
        ...
        public ActionResult ClassList(int currentpage = 1, int ClassId = 0, long del = 0)
        {
            ...
            using (chefuzi_dataEntities myOperating = new chefuzi_dataEntities())//数据库操作
            {
                ...
                if (del > 0)
                {
                    Child_Book_Click_Class myDelRecord = myOperating.Child_Book_Click_Class.FirstOrDefault(p => p.BookClassID == del);
                    if (myDelRecord != null)
                    {
                        myOperating.Child_Book_Click_Class.Remove(myDelRecord);
                        myOperating.SaveChanges();
                    }
                }
                #endregion
                ...
                //
                #region 取出内容
                IQueryable<Child_Book_Click_Class> myIQueryable = null;
                if (ClassId > 0)
                {
                    myIQueryable = myOperating.Child_Book_Click_Class.Where(p => p.BookClassID == ClassId);
                }
                else
                {
                    myIQueryable = myOperating.Child_Book_Click_Class;
                }
                //
                if (myIQueryable != null)
                {
                    List<Child_Book_Click_Class> BookTable = QueryableExtensions.OrderBy(myIQueryable, orderbyfiled, out recordCount, out pageCount, ref sqlCurrentpage, sqlPagesize, true).ToList();
                    //
                    ViewBag.DataList = BookTable;
                    #region 页数取值
                    ViewBag.CurrentPage = sqlCurrentpage;
        ...
        //
        #region 类别添加-编辑
        public ActionResult ClassAdd(BookClickClassModel model, string ReturnUrl, Nullable<int> myid = 0)
        {
            #region 获取来路路径
            if (System.Web.HttpContext.Current.Request.UrlReferrer != null)
            {
            ...
            using (chefuzi_dataEntities myOperating = new chefuzi_dataEntities())//数据库操作
            {
                if ((myid > 0) && (model.Operate == null))
                {
                    ...
                    //
                    #region 取出数据
                    Child_Book_Click_Class editRecord = myOperating.Child_Book_Click_Class.FirstOrDefault(p => p.BookClassID == mySelfId);
                    if (editRecord != null)
                    {
                        model.BookClassID = editRecord.BookClassID;
                ...
                else if (model.Operate == "add")
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        Child_Book_Click_Class addRecord = new Child_Book_Click_Class();
                        addRecord.BookClassID = model.BookClassID;
                        ...
                        //
                        myOperating.Child_Book_Click_Class.Add(addRecord);
                        myOperating.SaveChanges();
                        model = null;
                        //
                        return RedirectToLocal(ReturnUrl);
                ...
                else if (model.Operate == "edit")
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        Child_Book_Click_Class editSaveRecord = myOperating.Child_Book_Click_Class.FirstOrDefault(p => p.BookClassID == model.BookClassID);
                        if (editSaveRecord != null)
                        {
                ...
                else
                {
                    ...
                    try
                    {
                        model.BookClassID = myOperating.Child_Book_Click_Class.Max(p => p.BookClassID) + 1;
                        model.OrderBy = myOperating.Child_Book_Click_Class.Max(p => p.OrderBy) + 1;
                    }
                    catch
                    {
                        model.OrderBy = 0;
